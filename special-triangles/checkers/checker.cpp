...
inline void readAndCheckAnswer(InStream &in, TResult r) {
    int result = in.readInt(-1, 1);

    if (result == -1) {
        if ((a * 2) % b == 0) {
            in.quitf(r, "Such a triangle exists");
            exit(0);
        }
    }

    else if (result == 1) {
        unsigned long long int ans[6];

        if ((a * 2) % b != 0) {
            in.quitf(r, "No such triangle exists");
            exit(0);
        }

        for (int i = 0; i < 6; i++) ans[i] = in.readLong(0LL, 4'000'000'000LL);

        for (int i = 0; i <= 5; i++) {
            if (ans[i] == ans[(i + 2) % 6]) {
                in.quitf(
                    r, "No two points should have the same x or y coordinate");
                exit(0);
            }
        }
        ...
        for (int i = 0; i <= 5; i += 2) {
            p += BigInt::BigInt(ans[i]) * BigInt::BigInt(ans[(i + 3) % 6]);
        }

        for (int i = 1; i <= 5; i += 2) {
            p -= BigInt::BigInt(ans[i]) * BigInt::BigInt(ans[(i + 1) % 6]);
        }
        ...
        if (p != abs((2 * a) / b)) {
            in.quitf(r, "Area is not equal to a/b");
            exit(0);
        }

    } else {
        in.quitf(r, "The first line should be 1 or -1");
        exit(0);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    readAndCheckAnswer(ans, _fail);
    readAndCheckAnswer(ouf, _wa);
