...
bool dominates(int a, int b, int x, int y) { return a >= x && b >= y; }
...
void readAns(bool isParticipant, InStream& stream) {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (s.find(make_pair(x, y)) == s.end()) {
            quitf(_wa, "answer is not in the given list of coordinates");
        }

        if (dominates(x, y, queries[i].first, queries[i].second)) {
            continue;
        } else {
            quitf(_wa, "problem condition not satisfied");
        }
    }
}
