...
int main() {
    ...
    CoordinatesList a(n);
    for (auto &&[x, y] : a) IO::read_int(x, y);
    ...
    std::function<CoordinatesList(const CoordinatesList &)> find_maximal =
        ...
        [&find_maximal](const CoordinatesList &a) {
            ...
            auto l_maximal =
                find_maximal(CoordinatesList(begin(a), begin(a) + n / 2));
            auto r_maximal =
                find_maximal(CoordinatesList(begin(a) + n / 2, end(a)));
            ...
            for (const auto &[x, y] : l_maximal)
                if (y > max_second) r_maximal.emplace_back(x, y);
