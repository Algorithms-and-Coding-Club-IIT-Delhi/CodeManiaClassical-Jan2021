...
int main() {
    ...
    std::function<void(const i32&, const i32&)> find_maximal =
        ...
        [&](const i32& l, const i32& r) {
            ...
            for (int i = m + 1; i <= r; ++i)
                if (possible[i]) max_second = std::max(max_second, a[i].second);

            for (int i = l; i <= m; ++i)
                if (possible[i] && a[i].second <= max_second) possible[i] = 0;
        };

    find_maximal(0, n - 1);
    i32 total = 0;
    for (i32 i = 0; i < n; ++i) total += possible[i];
    CoordinatesList maximal_vectors(total);
    for (i32 i = 0; i < n; ++i)
        if (possible[i]) maximal_vectors[--total] = a[i];

#if DEBUG

    std::cerr << "Maximal vectors" << std::endl;

    for (auto&& [x, y] : maximal_vectors) std::cerr << x << ' ' << y << '\n';
    ...
    while (q--) {
        ...
        if (dominating_index == -1) {
            ...
            if (dominates(maximal_vectors[dominating_index],
                          std::make_pair(x, y))) {
